...
public protocol RearrangerDelegate: class {
  ...
  func rearranger(_ rearranger: Rearranger, shouldMoveSectionAt source: IndexPath) -> Bool
  
  /// 移动Section时才会回调，会频繁调用
  /// - Parameter rearranger: 排序处理器
  /// - Parameter source: 移动中Section的起始索引
  /// - Parameter destination: 移动中Section的目标索引
  func rearranger(_ rearranger: Rearranger, moveSectionFrom source: IndexPath, to destination: IndexPath)
